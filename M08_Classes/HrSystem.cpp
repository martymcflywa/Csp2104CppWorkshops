...
void HrSystem::read()
{
    ...
    while (getline(file, line))
    {
        ...
        _employees.push_back(Employee(id, name, role, rate));
        recordLine = 0;
    }
...
{
    cout << "Managers:" << endl;
    auto managers = filter([](const Employee& e)
    {
        return e.getRole() == "manager";
    });
...
{
    cout << "Workers:" << endl;
    auto workers = filter([](const Employee& e)
    {
        return e.getRole() == "worker";
    });
...
void HrSystem::printEmployee(Employee employee)
{
